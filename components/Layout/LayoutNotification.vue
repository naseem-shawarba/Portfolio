<template>
  <div class="text-center">
    <v-snackbar v-model="snackbar" location="top" :timeout="-1" :color="`${color}-darken-1`">
      {{ text }}
      <template v-slot:actions>
        <v-btn v-bind="attrs" @click="snackbar = false" icon="mdi-close" :color="`${color}-lighten-4`" />
      </template>
      <v-progress-linear v-model="progressPercentage" :color="`${color}-lighten-2`" />
    </v-snackbar>
  </div>
</template>
